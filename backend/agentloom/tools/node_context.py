...
class GetNodeContextTool(Tool):
    ...
    description = (
        "Retrieve the verbatim original content of any earlier "
        "ChatNode or WorkNode by its id. **USE THIS TOOL whenever the "
        "user asks for specific details from earlier in the "
        "conversation that may have been compacted or packed away** "
        "— exact numbers, specific names, verbatim quotes, parameter "
        "values, prior facts. Pack/compact summaries in your context "
        "cite source ids in `[node:xxx]` or `(节点: xxx)` form; pass "
        "those ids verbatim as the `node_id` argument. **Do NOT "
        "answer from memory of a summary alone when the user asks "
        "for verbatim content** — summaries are lossy and may have "
        "dropped the specific detail the user needs. "
        "Returns: for a ChatNode, just that turn's user_message + "
        "agent_response (no ancestors); for a WorkNode, "
        "input_messages + output_message + the enclosing WorkFlow's "
        "trio (description / inputs / expected_outcome) and any "
        "tool_call fields. Both kinds include parent_ids for "
        "upstream walks. Search scope defaults to the current "
        "ChatFlow; pass scope='cross_chatflow' (and hold the matching "
        "virtual capability) to look up nodes in sibling ChatFlows. "
        "Oversized responses are truncated at max_bytes (default "
        "~50 KiB)."
    )
    parameters = {
        "type": "object",
