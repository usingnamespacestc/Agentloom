...
class ChatFlowEngine:
    ...
    async def pack_workflow_range(
        ...
        *,
        ...
        """User-initiated pack over a WorkNode range.

        Resolves ``chat_node_id`` (+ optional nested ``sub_path`` into
        delegate sub-workflows) to an inner WorkFlow, materializes a
        PACK WorkNode via ``_build_pack_worknode`` with
        ``parent_ids = [packed_range[-1]]``, then runs the WorkFlow to
        drive the pack to completion. The newly-added pack is the only
        PLANNED node at call time, so ``execute`` only runs the pack.

        The frozen pack node is returned.
        """
        runtime = self._require_runtime(chatflow_id)
        sub_path = list(sub_path or [])

        async with runtime.lock:
            chatflow = runtime.chatflow
            chat_node = chatflow.nodes.get(chat_node_id)
            if chat_node is None:
                raise KeyError(f"chat node {chat_node_id!r} not in chatflow")
            wf = chat_node.workflow
            for wn_id in sub_path:
                wn = wf.nodes.get(wn_id)
                if wn is None or wn.sub_workflow is None:
                    raise KeyError(
                        f"work node {wn_id!r} has no sub_workflow on the resolved path"
                    )
                wf = wn.sub_workflow

            pack_node = self._inner._build_pack_worknode(
                wf,
                packed_range,
        ...
        try:
            await self._inner.execute(
                ...
                chatflow_ground_ratio_grace_nodes=20,
                # Pack is a user-initiated single-shot summarizer; auto-
                # compacting its own input would recurse uselessly.
                chatflow_compact_trigger_pct=None,
                chatflow_id=chatflow.id,
                disabled_tool_names=effective_disabled,
            )
        except Exception as exc:  # noqa: BLE001 — engine boundary
            log.exception("pack %s inner workflow raised", pack_id)
            async with runtime.lock:
                node = target_wf.nodes.get(pack_id)
                if node is not None:
        ...
        await self._save(runtime)
        async with runtime.lock:
            return target_wf.nodes[pack_id]
