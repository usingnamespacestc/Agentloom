...
## 🛡️ 第八轮：decompose fact-loss 5 层防御 ship 完（commit `b063e61..5f89681`）

第七轮列出的 prong 2-6 全部落地。原本的"判官 + planner 共同盲点"在引擎和 prompt 两侧各自都有兜底，**普通用户不再需要懂内部机制就能避开**。

| Commit | Prong | 修了什么 |
|---|---|---|
| `b063e61` | 🟢 5 (root cause) | **WorkFlow 自带 `outer_user_message`，`_build_sub_workflow_for_subtask` 自动注入 sub 的 judge_pre**：top-level `_spawn_turn_node` 把原始 user_message 写进 WorkFlow，每层 sub-WorkFlow 沿继承链复制；spawn sub 时 prepend 一条带 "[Outer ChatFlow context]" 标签的 user 消息到 judge_pre 的 input_messages。最深层 sub-worker 也能看到字面 SQL / 代码 / 数据。**不依赖 model 自觉**。docstring 解释 trade-off + bilingual preamble |
| `bcc0e4b` | 🟢 6 (re-plan 边界) | **每个 `_spawn_planner` 调用都 append outer_user_message preamble**：判 judge_pre 把 user 字面材料 paraphrase 进 trio.inputs 时，所有 planner（initial / parse-error retry / phantom-tool retry / plan_judge revise / capability-driven re-plan）都拿得到 user 原话。append 在 fixture 自有的 user msg 之后，不破坏 system→user 顺序 |
| `3163972` | 🟢 2 + 3 | **`<missing_input>` worker marker + engine 反馈 → 重新 plan**：worker 主动报"我看不见 X"信号（XML tag form + JSON segment form，对称 `capability_request`）→ engine `_extract_missing_input` 落到 `WorkFlowNode.missing_input` → worker_judge / judge_post 把它 bubble 进 `JudgeVerdict.missing_input_escalation` → `_after_worker_judge` 看到非空就**spawn fresh planner**带 handoff_notes（"上一轮 worker 反馈缺 X，新 brief 必须 verbatim inline"），不走 retry / fail。共享 `debate_round_budget` 不引入新 budget。worker_judge fixture 中英双语都加了 missing-input 教育段。15 unit tests 钉住 parser + schema 各形态 |
| `5f89681` | 🟢 4 (delegation cascade) | **delegation 失败也走 re-plan，不直接 aggregate fail**：`_after_delegation` 看到 decompose group 全部 terminal 后，先扫每个 FAILED member 的 sub-WorkFlow `judge_post.missing_input_escalation`；任何一条非空 → spawn fresh planner 在 owner（planner_judge）下，handoff_notes union 所有 gap 描述，重写 brief 把缺失材料 inline。比 aggregator 拿 partial-fail 给用户更"诊断"得多。`_collect_missing_input_from_failed_members` helper + 8 unit tests |

### 5 层防御整体形态

针对"用户贴数据 + 多任务 prompt → planner 拆解时丢字面材料 → sub-worker 装糊涂"这条最常见的 UX bug：

1. **plan + plan_judge prompt 教育**（`91830f8`，第七轮）—— 模型层面知道有 brief 边界，✗/✓ 范例
2. **`<missing_input>` 信号 + judge bubble**（`3163972`）—— worker 漏看见结构化说出来
3. **engine 收信号 → 重新 plan**（`3163972`）—— 不让漏的事重来同一份 plan
4. **delegation cascade**（`5f89681`）—— 失败的 sub 触发父层 re-plan，不只是 fail
5. **engine 自动注入 user_message**（`b063e61` + `bcc0e4b`）—— 两个边界都自动塞，**不靠模型自觉**

任何一层失效，下一层接住。**普通用户走 happy path**：要么 prompt 教育让模型自己写对（1），要么模型自觉报 missing_input 信号触发重新规划（2-4），要么完全不靠模型——engine 自动把数据塞到 sub-WorkFlow 看得到的地方（5）。

909 backend tests pass.
